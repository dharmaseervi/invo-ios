...
class EditInvoiceViewModel: ObservableObject {
        ...
        for item in invoiceItems {
            do {
                ...
                if let itemDetail = itemResponse.first {
                    ...
                    // Fallback: create placeholder item if not found
                    loadedItems.append(createPlaceholderLineItem(from: item))
                }
            } catch {
                // Fallback on error
                loadedItems.append(createPlaceholderLineItem(from: item))
    ...
    private func createPlaceholderLineItem(
        ...
        let placeholderItem = ItemResponse(
            ...
            category_id: 0,
            hsn_code: "hsn",
            sku: String(format: "%.2f", item.rate),
            unit: String(item.tax_rate),
            description: "This item was deleted or is unavailable",
            cost_price: 0,
            price: 0,
            quantity: 0,
            low_stock_alert: 0,
